...
- **当前暂用 `TOKENS_PER_PCT = 1_500_000`（1% ≈ 150 万 token）**，区间 **50 万 ~ 300 万**。
- 为什么是暂定：
  - 豆包 timeline API **只返回百分比**（无绝对 token 字段），配额接口也只返回 `used_percent`，
    本地 / API 均无“一个 7 天窗口 = 多少 token”的硬锚点，故无法做 1:1 精确校准。
  - **下界证据**：仅 agent 模式轨迹重建 = **2.385 亿 token**（1595 条 timeline = 499.2%），
    得 1% ≥ 47.8 万；但 agent 模式只是用量的一部分，真实系数必 > 50 万。
  - **已证伪**：commit `90c788c` 曾误改为 `5_000_000`（10 倍偏差，无依据），已回退；500 万肯定错。
  - 真实系数大概率落在 **100 万 ~ 300 万**（重度使用：已烧满 5 个 7 天窗口、单日峰值 79%）。
