...
- **1% ≈ 50 万 token**（`TOKENS_PER_PCT = 500_000`）
- 校准依据（双向交叉验证，2026-09-21 复核）：
  - timeline API 全量 **1595 条 = 499.2%**
  - 会话轨迹重建（agent 模式上下文累积下界）**2.385 亿 token**
  - 两种算法均得 **≈47.8 万/1%**；计入 system prompt + 工具定义固定开销后约 **50~56 万/1%**
  - 与豆包 ¥68/月包月定价量级吻合（100% ≈ 5000 万 token/周期）
- ⚠️ 历史坑：commit `90c788c` 曾误将系数改为 `5_000_000`（10 倍偏差），导致用量虚高，已回退。
