...
- 唯一可靠的测法是**消息级对齐**：timeline 每条 = 一条 user 消息，本地重建按同一批消息
  ...
  但上界不用猜，窗口约束能算出来：模型窗口 = **256K**
  （doubao-seed-2-1-turbo / -pro 均为 256K；本地配置出现的模型也只有这两个系列，
  没有 1M 窗口模型）。实测 max 单次上下文 = 240,688，加 system prompt 后
  距 256K 只剩 **18,785 tok** —— 这就是全部「看不见的消耗」每次能占的总预算。
  ⇒ **真值区间 50 ~ 58.9 万（两头都硬，闭合），取整建议 55 万。**
  详见 README「已知系统性偏差」一节。
...
#   区间经窗口约束闭合为 **50 ~ 58.9 万**（上界由 max 上下文 240,688 + 256K 窗口推出），
#   取整建议 **55 万**。详见 README「已知系统性偏差」。
